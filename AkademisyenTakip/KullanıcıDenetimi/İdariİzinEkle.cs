...
{
    public partial class İdariİzinEkle : UserControl
    {
        public İdariİzinEkle()
        {
            InitializeComponent();
        }
        ...
        private void iekle_btn_Click(object sender, EventArgs e)
        {
            ...
            dbclass.com = new SqlCommand("Select * from IdariIzin where Id=" + oid, dbclass.con);
            if (dr2.Read())
            {
            ...
            dr2.Close();
            if (kontrol == true) dbclass.com = new SqlCommand("INSERT INTO IdariIzin(ID,IzinGun,BaTarihi,BiTarihi) values('" + oid + "','" + izinbslngc.Value.ToString("dddd")+"','"+izinbslngc.Value.ToString("yyyy-MM-dd")+"','" + izinbts.Value.ToString("yyyy-MM-dd") + "')", dbclass.con);
            dbclass.com.ExecuteNonQuery();
            dbclass.con.Close();
        }

        private void İdariİzinEkle_Load(object sender, EventArgs e)
        {
            ...
            if (dbclass.con.State == ConnectionState.Closed)
                dbclass.con.Open();
            dbclass.com = new SqlCommand("SELECT i.ID,DATEDIFF(day,i.BaTarihi,İ.BiTarihi) as Gün FROM IdariIzin i WHERE i.BiTarihi<@tarih", dbclass.con);
            ...
            dr.Close();
            dbclass.com = new SqlCommand("Delete From IdariIzin where BiTarihi<@tarih", dbclass.con);
            dbclass.com.Parameters.AddWithValue("@tarih", DateTime.Now.ToString("yyyy-MM-dd"));
            dbclass.com.ExecuteNonQuery();
            for (int i = 0; i < id.Count; i++)
            {
                dbclass.com = new SqlCommand("update OgretmenBilgileri set KullanılanIdariIzin=@kigun where ID=@id", dbclass.con);
